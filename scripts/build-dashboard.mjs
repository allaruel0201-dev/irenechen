...
function extractFirstUrl(text) {
  const markdownLink = String(text || "").match(/\((https?:\/\/[^)\s]+)\)/);
  if (markdownLink) return markdownLink[1].replace(/\)$/, "");
  const rawUrl = String(text || "").match(/https?:\/\/\S+/);
  return rawUrl ? rawUrl[0].replace(/\)$/, "") : "";
}
...
function parseAlternativeTopics(markdown) {
  ...
  if (tableLines.length >= 3) {
    ...
    for (const line of tableLines.slice(2)) {
      ...
      const [title, summary, source, score] = cols;
      ...
      alternatives.push({
        ...
        source_url: extractFirstUrl(source),
      });
    }
  ...
  while (i < lines.length) {
    ...
    if (blockStart) {
      ...
      for (const blockLine of block) {
        ...
        if (!sourceUrl) {
          sourceUrl = extractFirstUrl(blockLine);
        }
        const scoreMatch = blockLine.match(/(?:评分|总分)[：:]\s*(\d{1,2})\s*$/);
    ...
    if (!summary) {
      for (const blockLine of block) {
        if (!blockLine || blockLine.startsWith("来源：")) continue;
        const totalMatch = blockLine.match(/总分[：:]\s*(\d{1,2})\s*(?:\/\s*30)?/);
        if (totalMatch) {
          scoreTotal = Number(totalMatch[1]);
          continue;
        }
        summary = blockLine;
        break;
...
async function refreshDashboardAssetVersions(version) {
  try {
    const text = await fs.readFile(DASHBOARD_INDEX, "utf8");
    const updated = text
      .replace(/(\.\/style\.css\?v=)[^"]+/g, `$1${version}`)
      .replace(/(\.\/data\.js\?v=)[^"]+/g, `$1${version}`)
      .replace(/(\.\/app\.js\?v=)[^"]+/g, `$1${version}`);
    if (updated !== text) {
      await fs.writeFile(DASHBOARD_INDEX, updated, "utf8");
    }
  } catch {
    // Missing index should not block data generation.
  }
}
...
async function build() {
  ...
  const version = data.generated_at.replace(/[^0-9A-Za-z]/g, "");
  ...
  await refreshDashboardAssetVersions(version);
}
