...
ASSET_VERSION_PATTERNS = (
  (re.compile(r"(\./style\.css\?v=)[^\"]+"), r"\g<1>{version}"),
...
SERVICE_WORKER_REGISTRATION = """    <!-- OFFLINE_FALLBACK_START -->
    <script>
...
def refresh_dashboard_asset_versions(version: str) -> None:
  ...
  updated = text
  for pattern, replacement in ASSET_VERSION_PATTERNS:
    updated = pattern.sub(replacement.format(version=version), updated)
...
def ensure_offline_fallback() -> None:
  """Register a service worker so the latest successful page survives CDN outages."""
  index_path = DASHBOARD_DIR / "index.html"
  if not index_path.exists():
    return
  ...
  updated = text.replace("</body>", f"{SERVICE_WORKER_REGISTRATION}  </body>", 1)
  index_path.write_text(updated, encoding="utf-8")


def write_service_worker(version: str) -> None:
  """Cache the atomic dashboard page and fall back to it on navigation failure."""
  script = f"""// Auto-generated by scripts/build-dashboard.py
const CACHE_NAME = "topic-radar-{version}";
...
"""
  (DASHBOARD_DIR / "sw.js").write_text(script, encoding="utf-8")
...
def sync_root_dashboard_fallback() -> None:
  ...
  for name in ("index.html", "app.js", "style.css", "data.js", "sw.js"):
...
def build() -> None:
  ...
  version = re.sub(r"[^0-9A-Za-z]", "", data["generated_at"])
